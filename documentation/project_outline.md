...
This repository is a local prototype for **Orgarythmus**, a project/goal planning app. It combines a FastAPI + SQLite backend with a Vite/React frontend. The product shape is: create projects, add goals, classify goals into dimensions/categories, and plan them on a Gantt-like schedule with milestones, dependencies, deadlines, filters, perspectives, undo, and redo.

## Runtime shape

- Backend: `main.py`
  - FastAPI app on `http://localhost:8000`.
  - SQLite database at `goals.db`.
...
3. The workspace has four horizontally sliding views:
   ...
   - Goals: quick goal capture / brainstorming.
   - Classification: goal grouping, coloring, dimensions/categories, filters, perspectives.
   - Schedule: Gantt-style milestone planning, dependencies, deadlines, filters, perspectives, transactions.
4. Goal details open through `GoalPopup`.

`App.jsx` owns the top-level screen state, active project, current workspace view, goal list, quick-add behavior, and popup/toast plumbing.
...
- `src/App.jsx`
  - Top-level React state machine: home vs workspace, active project, four workspace views, goal popup, quick-add toast.
...
- `src/components/BrainstormV2.jsx`
  - Goal capture / brainstorming surface.
...
- `src/components/GoalPopup.jsx`
  - Goal editor popup and delete/update integration.
- `src/components/CategoryAssignmentPicker.jsx`
  - Category assignment UI used around goal editing.
...
- `pages`
  - Goals. Historical naming still says "pages"; in the UI these are goals.
...
- `assignments`
  - Goal-to-category assignment per dimension.
  - Primary key is `(goal_id, dimension_id)`, so one goal has at most one category per dimension.
- `milestones`
  - Schedule blocks tied to a goal row: start column, duration, title, color.
- `dependencies`
  - Directed milestone relationships: `from_id` must finish before `to_id` starts.
  - Includes an optional reason.
- `deadlines`
  - One hard deadline column per goal.
- `saved_filters`
...
- Goals/pages: `/pages`, `/pages/{page_id}`, `/pages/order`
- Dimensions/categories: `/dimensions`, `/dimensions/{dim_id}`, `/dimensions/{dim_id}/categories`, `/categories`, `/categories/order`
- Assignments: `/assignments`, `/goals/{goal_id}/assign/{dim_id}`, `/assignments/order`
- Filters: `/filters`
...
- Milestones in the same goal row do not overlap.
- Milestones in the same goal row do not pass each other when both existed before and after.
...
- Assignments map goals into dimensions.
...
- Milestones in the same goal cannot overlap.
- Milestones cannot exceed their goal deadline.
- Dependencies require predecessor end to be before successor start.
- Dependency cycles are invalid.

Frontend warning behavior in `SchedulePage` includes state for revealed conflict goals, pending conflict milestones, warning prompts, blinking dependencies/milestones, dependency conflict resolution selection, and temporary view changes to make hidden conflicts visible.

The central design tension is the "collapse/expand dilemma": conflicts may be hidden by collapsed lanes, collapsed goals, quick filters, named filters, or perspective state. The documented intended approach is to snapshot view state, reveal conflicts temporarily, then restore the previous perspective-like state after a short delay.
...
- `goals.db` is tracked/present in the repo and can be modified by local app usage. Be careful not to commit accidental data changes unless they are intended.
...
- Naming is mixed in places: "pages" in backend/API means "goals" in the product UI.
...
- Add a project-scoped feature:
  - Ensure backend queries filter by `project_id` either directly or through project-owned goals/dimensions.
...
1. Projects and goals provide the base content.
