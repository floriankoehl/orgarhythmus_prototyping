...
**Project scoping:** `pages`, `dimensions`, `saved_filters`, and perspectives have `project_id` directly. `categories`, `milestones`, `dependencies`, `deadlines` don't — they're reached via subqueries (e.g. `WHERE goal_id IN (SELECT id FROM pages WHERE project_id = ?)`).
...
**`App.jsx`:** Two screens: `home` (project list) or `workspace` (four views). State: `view` (0–3), `activeProject`, `goals`. All four views are always mounted — `display: flex | none` to switch. This keeps Classification and Schedule state alive when navigating away.

**Views:**
- `0` — ProjectDashboard: metadata, stats, export
- `1` — BrainstormV2: goal capture
...
- `buildRowItems()` produces flat row model: lane headers, goal rows, gaps
- Conflict reveal: when a transaction violates constraints, temporarily unhides the conflicting goals, blinks milestones, then restores prior view state
...
| `src/App.jsx` | Screen routing, goal list, popup, quick-add |
